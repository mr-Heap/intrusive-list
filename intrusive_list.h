...
struct list_base {
  ...
  list_base(intrusive::list_base* prev, intrusive::list_base* next)
      : prev(prev), next(next) {}
...
public:
  ...
  public:
    ...
    explicit my_iterator(list_base* it) : it(it) {}
  ...
  iterator insert(iterator next,
                  T& element) { // you can't insert an element twice
  ...
  void splice(const_iterator start, list<T>& from, const_iterator position_from,
